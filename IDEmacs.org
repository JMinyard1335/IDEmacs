...
#+title IDEmacs
remove the line Emacs-lisp :tangle ~/.emacs to keep this file from writing over a pre existing  ~/.emacs if you have your own config already set up.
...
Welcome to More formatting of text *YAY!*.  No for real tho in this section we have code that centers the org buffer on screen to make it closer to other document editorrs most people are fimilar with. For more infromation on visual-fill-column click [[https://github.com/joostkremers/visual-fill-column][here]].
...
Description:
[[https://orgmode.org/worg/org-contrib/babel/][org-babel]] allows for some awesome functionality with code blocks in org docs such as being able to evaluate and display the results of the code or writing the code to a separate source file as you develop. such as this file does with your Emacs config file.
...
** Treemacs
Treemacs is a file display system similar to something found in vs code. This is installed as a dependency of lsp-treemacs and all of its features I might put more effort into its customization later.
